...
def narrowband_wave_finder(freqs,psd,phase,fl_s=0.0016,fh_s=0.0067,fl_t=0.000278,fh_t=0.027778):
    """ 
    inputs: 
    ...
    if np.size(sig_ind) == 0:
        ...
        if  FWHM >= 2:
            ...
            I_sig = S_sig/S_total
            return FWHM,results_half[2][0],results_half[3][0], peak_psd_max,peak_freq,S_sig,S_total,I_sig,peak_ang
...
def save_event_info(fname='wave_events_info.csv',stack_plot=True,I_min=0.5,N_min=420):
    """ 
    ...
    event_dic = {'FWHM': [],'f_left_ind':[],'f_right_ind':[],'peak_psd':[],
                 'peak_freq':[],'peak_ang':[],'S_sig':[],'S_total':[],'I_sig':[],'mlat':[],
