...
class Stats(object):
    ...
    def __init__(
        ...
        for f in self.files:
            self.D = pd.concat([
                self.D, 
                pd.read_csv(f, parse_dates=parse_dates)
            ])
        self.D["Ndpint"] = self.D.len * self.D.intt
        self.D["jhr"] = self.D.op_ped * self.D.Erms**2 * 1000. #mW/m^2
        self.D.Erms = self.D.Erms*1000. # mV
    ...
    def get_valid_events(
        ...
        """
        Ndpint_threshold = Ndpint_threshold if Ndpint_threshold else self.Ndpint_threshold
        I_sig_trhreshold = I_sig_trhreshold if I_sig_trhreshold else self.I_sig_trhreshold
        logger.info(f"Fetch {key} events with ndp:{Ndpint_threshold} and iSig:{I_sig_trhreshold}")
        o = self.D.copy()
        o = o[
            (o.Ndpint >= Ndpint_threshold)&
            (o.I_sig > I_sig_trhreshold)
        ]
    ...
    def fetch_events_based_on_index(
        ...
        if len(index_limits) == 2:
            o = o[
                (o[index]<index_limits[0]) &
                (o[index]>=index_limits[1])
            ]
        else:
...
class TimeSeriesAnalysis(object):
    ...
    def __init__(
        ...
        self.event = event
        self.hours = hours
        self.stime = (self.event.stime - dt.timedelta(hours=self.hours)).replace(minute=0, second=0)
        self.etime = (self.event.etime + dt.timedelta(hours=self.hours+1)).replace(minute=0, second=0)
        self.load_omni()
    ...
    def load_omni(self, loc="tmp/data/omni/*.csv"):
        ...
        name_str = [
            self.stime.strftime("%Y%m"),
            self.etime.strftime("%Y%m")
        ]
        for f in files:
        ...
        self.omni = self.omni[
            (self.omni.DATE>=self.stime) &
            (self.omni.DATE<=self.etime)
        ]
        return
...
class StackPlots(object):
    ...
    def set_labels(self, ax, xlabel, ylabel, title, ycol="k"):
        ...
        ax.set_ylabel(ylabel, fontdict={"size": 12, "fontweight": "bold", "color":ycol})
    ...
    def plot_indexs(self):
        ...
        ax.plot(self.ts.omni.DATE, self.ts.omni.Bz_GSM, "bo", ms=0.8, ls="None", label=r"$B_z$")
        ax.plot(self.ts.omni.DATE, self.ts.omni.By_GSM, "ro", ms=0.8, ls="None", label=r"$B_y$")
        ax.plot(self.ts.omni.DATE, self.ts.omni.Bx, "ko", ms=0.8, ls="None", label=r"$B_x$")
        ax.legend(loc=2)
        ax.set_ylim(-20, 20)
    ...
    def addParamPlot(
        ...
        ax.set_ylim([0, 100])
        im = ax.pcolormesh(X, Y, Z.T, lw=0.01, edgecolors="None", cmap=cmap, vmax=pmax, vmin=pmin)
        ax.set_xlim(self.ts.stime, self.ts.etime)
        ax.axvline(self.ts.event.stime, color="gray", ls="-", lw=0.4)
        ax.axvline(self.ts.event.etime, color="gray", ls="-", lw=0.4)
